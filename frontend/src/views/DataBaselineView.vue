    ...
    <div class="space-y-5">
      ...
      <template v-if="activeTab === 'anggota'">
        ...
        <BaselineTable
          ...
          :error="anggota.error"
          :rows="anggota.rows"
          :columns="anggota.columns"
          :meta="anggota.meta"
          :history-stack="anggota.historyStack"
      ...
      <template v-if="activeTab === 'keluarga'">
        ...
        <BaselineTable
          ...
          :error="keluarga.error"
          :rows="keluarga.rows"
          :columns="keluarga.columns"
          :meta="keluarga.meta"
          :history-stack="keluarga.historyStack"
          empty-hint="Klik Tampilkan untuk memuat data keluarga."
          title="Data Keluarga"
          @next="nextKeluarga"
          @prev="prevKeluarga"
        />
...
async function loadAnggota(cursor = null) {
  ...
  try {
    ...
    anggota.rows    = res.data    ?? []
    anggota.columns = res.columns ?? []
...
function prevAnggota() {
  if (!anggota.historyStack.length) return
  loadAnggota(anggota.historyStack.pop())
}
...
async function loadKeluarga(cursor = null) {
  ...
  try {
    ...
    keluarga.rows    = res.data    ?? []
    keluarga.columns = res.columns ?? []
...
function prevKeluarga() {
  if (!keluarga.historyStack.length) return
  loadKeluarga(keluarga.historyStack.pop())
}
