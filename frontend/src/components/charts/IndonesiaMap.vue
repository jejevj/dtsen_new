<template>
  ...
  >
    ...
    <div v-if="hovered" style="position:absolute;top:16px;left:16px;background:white;border-radius:10px;box-shadow:0 4px 16px rgba(0,0,0,0.12);padding:14px;z-index:1000;min-width:190px;">
...
const mustahikLegend    = [
  { color:'#dcfce7', label:'< 30 rb' }, { color:'#86efac', label:'30–80 rb' },
  { color:'#4ade80', label:'80–150 rb' }, { color:'#16a34a', label:'150–300 rb' },
  { color:'#14532d', label:'> 300 rb' },
]
const penyaluranLegend  = [
  { color:'#dbeafe', label:'< 100 M' }, { color:'#93c5fd', label:'100–300 M' },
  { color:'#3b82f6', label:'300–600 M' }, { color:'#1d4ed8', label:'600 M–1 T' },
...
// GeoJSON CDN — URL valid per Juli 2026
...
function buildLookup() {
  const map = {}
  props.mapData.forEach(d => { if (d.provinsi_nama) map[normalize(d.provinsi_nama)] = d })
  return map
}
...
async function renderLayer() {
  ...
  geojsonLayer = L.geoJSON(geojson, {
    style: (feature) => {
      ...
      return { fillColor: getColor(val, scale), fillOpacity: val > 0 ? 0.85 : 0.25, color: '#ffffff', weight: 1.2, opacity: 1 }
    },
  ...
  if (geojsonLayer) {
    geojsonLayer.eachLayer(layer => {
      ...
      const raw  = feat?.properties?.state || feat?.properties?.name ||
                   ...
                   feat?.properties?.Propinsi || ''
      const data = props.mapData.find(d => normalize(d.provinsi_nama) === normalize(raw)) || {}
      layer.setStyle({ fillColor: getColor(data[props.metric]||0, scale), fillOpacity: data[props.metric] > 0 ? 0.85 : 0.25 })
    })
  }
}, { deep: true })
