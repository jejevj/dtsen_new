...
DATA_PROVINSI = """
    ...
    UNION
    SELECT DISTINCT ktp_provinsi_kode kode, m_provinsi.provinsi_nama nama
    FROM t_mustahik
    JOIN m_provinsi ON m_provinsi.provinsi_kode = ktp_provinsi_kode
    JOIN t_dtsen_akses ON t_dtsen_akses.laz_kode = t_mustahik.laz_kode
    WHERE t_dtsen_akses.email = :email
    AND YEAR(t_mustahik.tanggal_terima) = :tahun
"""
...
DATA_KABKOTA = """
    ...
    UNION
    SELECT DISTINCT ktp_kabkota_kode kode, m_kabkota.kabkota_nama nama
    FROM t_mustahik
    JOIN m_kabkota ON m_kabkota.kabkota_kode = ktp_kabkota_kode
    JOIN t_dtsen_akses ON t_dtsen_akses.laz_kode = t_mustahik.laz_kode
    WHERE t_dtsen_akses.email = :email
    AND YEAR(t_mustahik.tanggal_terima) = :tahun
"""
...
DATA_KECAMATAN = """
    ...
    UNION
    SELECT DISTINCT ktp_kecamatan_kode kode, m_kecamatan.kecamatan_nama nama
    FROM t_mustahik
    JOIN m_kecamatan ON m_kecamatan.kecamatan_kode = ktp_kecamatan_kode
    JOIN t_dtsen_akses ON t_dtsen_akses.laz_kode = t_mustahik.laz_kode
    WHERE t_dtsen_akses.email = :email
    AND YEAR(t_mustahik.tanggal_terima) = :tahun
"""
...
DATA_KELURAHAN = """
    SELECT DISTINCT m_kelurahan.kelurahan_kode kode, m_kelurahan.kelurahan_nama nama
    FROM m_kelurahan
    LEFT JOIN t_dtsen_wilayah ON t_dtsen_wilayah.provinsi_kode = LEFT(m_kelurahan.kecamatan_kode,2)
    JOIN t_dtsen_akses ON t_dtsen_akses.dtsen_akses_id = t_dtsen_wilayah.dtsen_akses_id
    WHERE (t_dtsen_wilayah.kecamatan_kode = m_kelurahan.kecamatan_kode OR t_dtsen_wilayah.kecamatan_kode IS NULL)
    AND t_dtsen_akses.email = :email
    AND m_kelurahan.kecamatan_kode = :kecamatan_kode
    UNION
    SELECT DISTINCT ktp_kelurahan_kode kode, m_kelurahan.kelurahan_nama nama
    FROM t_mustahik
    JOIN m_kelurahan ON m_kelurahan.kelurahan_kode = ktp_kelurahan_kode
    JOIN t_dtsen_akses ON t_dtsen_akses.laz_kode = t_mustahik.laz_kode
    WHERE t_dtsen_akses.email = :email
    AND YEAR(t_mustahik.tanggal_terima) = :tahun
"""
...
class Report:
    ...
    def get_param_lembaga() -> list:
        sql = text("""
        SELECT laz.laz_kode, UPPER(laz.laz_nama) laz_nama
        FROM v_laz_usulan_kma kma
        JOIN t_laz laz ON laz.laz_kode = kma.laz_kode
        WHERE kma.usulan_tipe IN ('pendirian','perpanjangan','daftar_ulang','naik skala')
        AND kma.laz_kode IS NOT NULL
        AND kma.kma_berkas IS NOT NULL
        AND kma.kma_tanggal <= CURRENT_DATE
        AND kma.kma_kadaluarsa >= CURRENT_DATE
        AND laz.skala != 0
        UNION
        SELECT uker_kode as laz_kode, UPPER(singkatan) laz_nama
        FROM m_uker
    ...
    def get_skala_laz() -> list:
        sql = text("""
        SELECT laz.skala, COUNT(DISTINCT laz.laz_kode) laz_count
        FROM v_laz_usulan_kma kma
        JOIN t_laz laz ON laz.laz_kode = kma.laz_kode
        WHERE kma.usulan_tipe IN ('pendirian','perpanjangan','daftar_ulang','naik skala')
        AND kma.laz_kode IS NOT NULL
    ...
    def get_desil_mustahik(params: dict) -> list:
        ...
        kelurahan_kode = params.get("kelurahan_kode") or None
        
        sql_text = """
        SELECT t_mustahik_bappenas.desil, COUNT(DISTINCT t_mustahik.nik) nik_count, SUM(t_mustahik.rupiah) rupiah
        FROM t_mustahik
        JOIN t_mustahik_bappenas ON t_mustahik.nik = t_mustahik_bappenas.nik
        ...
        if provinsi_kode:
            sql_text += " AND t_mustahik.ktp_provinsi_kode = :provinsi_kode "

        if kabkota_kode:
            sql_text += " AND t_mustahik.ktp_kabkota_kode = :kabkota_kode "

        if kecamatan_kode:
            sql_text += " AND t_mustahik.ktp_kecamatan_kode = :kecamatan_kode "

        if kelurahan_kode:
            sql_text += " AND t_mustahik.ktp_kelurahan_kode = :kelurahan_kode "
    ...
    def get_tren(params: dict) -> list:
        ...
        SELECT years.tahun, COALESCE(laporan.mustahik,0) mustahik, COALESCE(laporan.rupiah,0) Tren
        FROM years
        LEFT JOIN (
            SELECT YEAR(tanggal_terima) tahun, COUNT(DISTINCT nik) mustahik, COALESCE(SUM(rupiah),0) rupiah
            FROM t_mustahik
            GROUP BY YEAR(tanggal_terima)
        ) laporan ON laporan.tahun = years.tahun
        ORDER BY years.tahun
        """
        sql = text(sql_text)
    ...
    def get_gender(params: dict) -> list:
        ...
        sql_text = """
        SELECT t_mustahik.jenis_kelamin, IFNULL(COUNT(DISTINCT t_mustahik.nik), 0) mustahik
        FROM t_mustahik
        JOIN t_mustahik_bappenas ON t_mustahik.nik = t_mustahik_bappenas.nik
        JOIN m_provinsi ON m_provinsi.provinsi_kode = t_mustahik_bappenas.ktp_provinsi_kode
        JOIN m_kabkota ON m_kabkota.kabkota_kode = t_mustahik_bappenas.ktp_kabkota_kode
        JOIN m_kecamatan ON m_kecamatan.kecamatan_kode = t_mustahik_bappenas.ktp_kecamatan_kode
        JOIN m_kelurahan ON m_kelurahan.kelurahan_kode = t_mustahik_bappenas.ktp_kelurahan_kode
    ...
    def get_bidang(params: dict) -> list:
        ...
        LEFT JOIN (
            ...
            FROM t_program p
            JOIN t_mustahik m ON m.program_kode = p.program_kode
            JOIN m_provinsi ON m_provinsi.provinsi_kode = m.ktp_provinsi_kode
            JOIN m_kabkota ON m_kabkota.kabkota_kode = m.ktp_kabkota_kode
            JOIN m_kecamatan ON m_kecamatan.kecamatan_kode = m.ktp_kecamatan_kode
            JOIN m_kelurahan ON m_kelurahan.kelurahan_kode = m.ktp_kelurahan_kode
        ...
        if provinsi_kode:
            sql_text += " AND m.ktp_provinsi_kode = :provinsi_kode "

        if kabkota_kode:
            sql_text += " AND m.ktp_kabkota_kode = :kabkota_kode "

        if kecamatan_kode:
            sql_text += " AND m.ktp_kecamatan_kode = :kecamatan_kode "

        if kelurahan_kode:
            sql_text += " AND m.ktp_kelurahan_kode = :kelurahan_kode "
    ...
    def get_map_provinsi(params: dict) -> list:
        ...
        sql = text("""
            SELECT t_mustahik.ktp_provinsi_kode, m_provinsi.provinsi_nama, SUM(t_mustahik.rupiah) penyaluran, COUNT(DISTINCT t_mustahik.nik) mustahik, COUNT(DISTINCT t_mustahik.laz_kode) laz_count
            FROM t_mustahik
            LEFT JOIN t_mustahik_bappenas ON t_mustahik.mustahik_id = t_mustahik_bappenas.mustahik_id AND t_mustahik_bappenas.desil BETWEEN 1 AND 4
            JOIN m_provinsi ON m_provinsi.provinsi_kode = t_mustahik.ktp_provinsi_kode
            WHERE t_mustahik.tanggal_terima >= :start_date
            AND t_mustahik.tanggal_terima < :end_date
    ...
    def get_map_kabkota(params: dict) -> list:
        ...
        sql = text("""
            SELECT t_mustahik.ktp_provinsi_kode, t_mustahik.ktp_kabkota_kode, m_kabkota.kabkota_nama, SUM(t_mustahik.rupiah) penyaluran, COUNT(DISTINCT t_mustahik.nik) mustahik, COUNT(DISTINCT t_mustahik.laz_kode) laz_count
            FROM t_mustahik
            LEFT JOIN t_mustahik_bappenas ON t_mustahik.mustahik_id = t_mustahik_bappenas.mustahik_id AND t_mustahik_bappenas.desil BETWEEN 1 AND 4
            JOIN m_kabkota ON m_kabkota.kabkota_kode = t_mustahik.ktp_kabkota_kode
            WHERE t_mustahik.tanggal_terima >= :start_date
...
# ==============================================================================================================================
    ...
    def get_param_laz(params: dict) -> list:
        ...
        sql = text("""
        SELECT laz_kode kode, laz_nama nama
        FROM (
            SELECT laz.laz_kode, UPPER(laz.laz_nama) laz_nama
            FROM v_laz_usulan_kma kma
            JOIN t_laz laz ON laz.laz_kode = kma.laz_kode
        ...
        WHERE laz_kode = COALESCE((SELECT t_dtsen_akses.laz_kode FROM t_dtsen_akses WHERE email = :email), laz_kode)
        ORDER BY nama;
        """)
    ...
    def get_param_provinsi(params: dict) -> list:
        ...
        sql = text(DATA_PROVINSI + " ORDER BY nama; ")
    ...
    def get_param_kabkota(params: dict) -> list:
        ...
        sql = text(DATA_KABKOTA + " ORDER BY nama; ")
    ...
    def get_param_kecamatan(params: dict) -> list:
        ...
        sql = text(DATA_KECAMATAN + " ORDER BY nama; ")
    ...
    def get_data_bidang(params: dict) -> list:
        ...
        (
            ...
            m_provinsi.provinsi_nama lvl_1, '-' lvl_2, '-' lvl_3, m_provinsi.provinsi_kode, null kabkota_kode, null kecamatan_kode
            FROM t_mustahik
            JOIN t_mustahik_bappenas ON t_mustahik_bappenas.nik = t_mustahik.nik AND t_mustahik_bappenas.desil BETWEEN 1 AND 4
            JOIN m_provinsi ON m_provinsi.provinsi_kode = t_mustahik.ktp_provinsi_kode
    ...
    def get_baseline_wilayah(params: dict) -> list:
        ...
        if kecamatan_kode:
            ...
            sql_base_table = """
                ...
                CONCAT(SUBSTRING(m_kelurahan.kelurahan_kode, 1, 2), '.', SUBSTRING(m_kelurahan.kelurahan_kode, 3, 2), '.', SUBSTRING(m_kelurahan.kelurahan_kode, 5, 2), '.', SUBSTRING(m_kelurahan.kelurahan_kode, 7, 4)) zawa
    ...
    def get_data_wilayah(params: dict) -> list:
        ...
        sql_base_table = """
            ...
            , m_provinsi.provinsi_nama lvl_1 , '-' lvl_2 , '-' lvl_3 , '-' lvl_4
            FROM t_mustahik
            JOIN t_mustahik_master ON t_mustahik_master.nik = t_mustahik.nik AND t_mustahik_master.desil BETWEEN 1 AND 4
            JOIN m_provinsi ON m_provinsi.provinsi_kode = t_mustahik.ktp_provinsi_kode
            JOIN t_dtsen_akses ON t_dtsen_akses.laz_kode = t_mustahik.laz_kode OR t_dtsen_akses.laz_kode IS NULL
            WHERE t_dtsen_akses.email = :email
            AND t_mustahik.tanggal_terima >= :tahun_awal AND t_mustahik.tanggal_terima < :tahun_akhir
        ...
        SELECT DISTINCT wilayah.lvl, wilayah.zawa, wilayah.kode, wilayah.nama, wilayah.lvl_1, wilayah.lvl_2, wilayah.lvl_3, wilayah.lvl_4
            , 0 baseline
            , (
                SELECT COALESCE(COUNT(datwil.mustahik),0) FROM t_dtsen_data_wilayah datwil
                WHERE datwil.kode = wilayah.kode AND datwil.desil BETWEEN 1 AND 4
